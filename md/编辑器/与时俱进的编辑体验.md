...
We'll always offer word completion, but for the rich [languages](/docs/languages/overview.md), such as JavaScript, JSON, HTML, CSS, Less, Sass, C# and TypeScript, we offer a true IntelliSense experience. 

我们将会提供代码补全，但对于丰富的[语言](/docs/languages/overview.md)，例如JavaScript, JSON, HTML, CSS, Less, Sass, C# 和 TypeScript，我们提供一个真正的智能感知的体验。
...
Out of the box, `kbstyle(.)`, `kbstyle(Tab)` or `kbstyle(Enter)` are accept triggers but you can also [customize these key bindings](/docs/customization/keybindings.md).

开箱即用，`kbstyle(.)`, `kbstyle(Tab)` or `kbstyle(Enter)` 是接收触发键的，但您也可以[自定义绑定这些键](/docs/customization/keybindings.md)。
